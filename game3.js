...
corgiGame.Game3.prototype = {
    ...
    buildWorld: function() {
        //create map/world
        this.background = this.game.add.tileSprite(0, 0, 1280, 620, 'background3');
    ...
    buildEnemies: function() {
        ...
        for(var i=0; i <this.totalEnemies; i++){
            var pup = this.enemyPups.create(this.world.width, this.rnd.integerInRange(0,this.world.height-100), 'enemyPup3', 'enemyPup3000');
    ...
    buildEnemyLasers: function() {
        ...
        this.enemyLasers.createMultiple(30, 'laser4');
    ...
    updateEnemy: function() {
        ...
        for(var i = 0; i<this.totalEnemies; i++){
            ...
            var shoot = this.rnd.integerInRange(1, 400);
            //limits the amount of shots the enemies can shoot 1 in 300 chance
            if(shoot == 1 && pup.canShoot){
                this.fireEnemyLaser(pup);
            }
            //possible pup goes invisible
            if(shoot == 399){
                pup.alpha = 0.0;
                pup.canShoot = false;
            }
            //possible pup becomes visible
            if(shoot == 398){
                pup.alpha = 1.0;
                pup.canShoot = true;
            }
